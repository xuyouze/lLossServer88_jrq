...
class BaseModel(ABC):
    ...
    def test(self):
        with torch.no_grad():
            ...
            for name in self.net_names:
                ...
                # self.output[:] = getattr(self, "output_%s" % name).cpu()
            com1 = self.output > 0.5
            com2 = self.attr > 0
            # com2 = self.attr[:, getattr(self, "attr_%s_index" % name)] > 0
    ...
    def get_model_class_balance_precision(self):
        bal_prec_pos = torch.zeros_like(self.tpr)
        bal_prec_neg = torch.zeros_like(self.tnr)
        for i in range(self.config.dataset_config.attribute_num):
            if self.pos_num[i] != 0:
                bal_prec_pos[i] = self.tpr[i] / self.pos_num[i]
            if self.pos_num[i] != self.get_validate_size():
                bal_prec_neg[i] = self.tnr[i] / (self.get_validate_size() - self.pos_num[i])

        return 1 / 2 * (bal_prec_pos + bal_prec_neg)
