...
class CelebADataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # upper_dir = os.path.join(self.config.data_root_dir, self.config.face_upper)
        # middle_dir = os.path.join(self.config.data_root_dir, self.config.face_middle)
        # lower_dir = os.path.join(self.config.data_root_dir, self.config.face_lower)

        face_whole = Image.open(os.path.join(whole_dir, self.image_names[index])).convert('RGB')
        # face_upper = Image.open(os.path.join(upper_dir, self.image_names[index])).convert('RGB')
        # face_middle = Image.open(os.path.join(middle_dir, self.image_names[index])).convert('RGB')
        # face_lower = Image.open(os.path.join(lower_dir, self.image_names[index])).convert('RGB')

        face_whole = self.transform(face_whole)
        # face_upper = self.transform(face_upper)
        # face_middle = self.transform(face_middle)
        # face_lower = self.transform(face_lower)
